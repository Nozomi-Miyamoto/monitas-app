...
def analyze_condition(api_key: str, condition: str, panel: dict) -> dict:
    ...
    user_prompt = f"""以下の入力から調査したいターゲット条件を読み取り、モニターパネルからの回収見込みを推計してください。

入力は箇条書きの条件でも、口語の質問・相談文でも構いません。
「〇〇って取れる？」「こういう条件どう？」のような質問形式でも、ターゲット条件を正確に解釈してください。
入力に複数の解釈が可能な場合は、調査実務として最も一般的・現実的な解釈を選んでください。
...
def page_calculation(api_key: str, panel: dict):
    ...
    st.caption("条件の箇条書きでも、口語・質問・相談文でもOKです。")
    condition = st.text_area(
        label="条件・質問を入力",
        label_visibility="collapsed",
        placeholder=(
            "【書き方はなんでもOK】\n"
            "例）新NISAで放置型投資をしてる人って何人くらい取れる？投資歴3年以内の人が対象です\n"
            "例）小学生以下の子どもがいる共働き世帯の母親を取りたいんだけど、どのくらい回収できる？\n"
            "例）IT業界の30〜40代エンジニア、製品開発担当。こういう条件だと難しい？"
        ),
        height=130,
    )
